...
function cambiarPestana(tab) {
    ...
    document.getElementById('form-gastos').classList.toggle('hidden', isVentas);

    document.getElementById('titulo-seccion').textContent = isVentas ? 'Historial de Ventas' : 'Registro de Gastos';
    document.getElementById('buscador').placeholder = isVentas ? 'Buscar cliente...' : 'Buscar gasto...';
    document.getElementById('btn-excel').classList.toggle('hidden', !isVentas);

    document.getElementById('btn-tab-ventas').className = isVentas
        ? 'px-5 py-2 rounded-lg text-xs font-bold bg-white text-blue-700 shadow-sm transition-all'
        : 'px-5 py-2 rounded-lg text-xs font-bold text-white opacity-70';
    document.getElementById('btn-tab-gastos').className = !isVentas
        ? 'px-5 py-2 rounded-lg text-xs font-bold bg-white text-rose-600 shadow-sm transition-all'
        : 'px-5 py-2 rounded-lg text-xs font-bold text-white opacity-70';
...
function actualizarInterfaz() {
    const isVentas = !document.getElementById('form-ventas').classList.contains('hidden');
    ...
    pedidosActuales.forEach((pedido) => {
        totalVentas += pedido.precio_total;
        if (pedido.entregado) {
            listos += 1;
        } else {
            pendientes += 1;
    ...
    if (isVentas) {
        ...
        const pedidosFiltrados = pedidosActuales.filter((pedido) => pedido.cliente.toLowerCase().includes(query));
        ...
        pedidosFiltrados.forEach((pedido) => {
            ...
            body.innerHTML += `
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-all text-left">
                    ...
                    <td class="px-7 py-5 text-center font-black text-blue-600 dark:text-blue-400">S/. ${pedido.precio_total.toFixed(2)}</td>
        ...
        header.innerHTML = '<th class="px-7 py-5 text-left">Gasto</th><th class="px-7 py-5 text-center">Monto</th><th class="px-7 py-5 text-center">Acción</th>';
        ...
        gastosFiltrados.forEach((gasto) => {
            ...
            body.innerHTML += `
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-all text-left">
                    ...
                    <td class="px-7 py-5 text-center font-black text-rose-500">S/. ${gasto.monto.toFixed(2)}</td>
